...
async function loadBookDetail(id) {
  ...
  bookData.value = null;
  try {
    console.log("Đang load sách với id:", id);
    ...
    const res = await axios.get(`http://localhost:3000/detectiveNovels/${realId}`);
    console.log("Dữ liệu sách nhận được:", res.data);
    if (!res.data) {
